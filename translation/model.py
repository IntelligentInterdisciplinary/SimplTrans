...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        if config.model=="mtransformer":
            ...
            value=self.V(value).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
        else:
            query=self.Q(query).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            key=self.K(key).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            value=self.V(value).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
